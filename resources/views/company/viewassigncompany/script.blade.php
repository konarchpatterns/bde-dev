     ...
     var table = $('#compnyrecords').DataTable({
      ...
      // autowidth: false,
       ...
       //  scrollX: true,//scroll horizontally
        ...
        ajax: "{{ route('company.asssignanydata',['id'=>'All']) }}",
        columnDefs: [
          ...
          @permission('view.company.name')
            {data: 'coid', name: 'coid',class:"fooid dt-center",width:'1%' },
          @endpermission
          @permission('view.company.name')
                    ...
                    if(data == null  || data == '')
                    {
                     ...
                     var obj=data.split(",");
                        ...
                        for (i = 0; i < obj.length; i++) {
                          ...
                          var cemail="'"+subdd[0]+"'";
                          text1 += '<span name=emailtext[] value="'+cemail+'"  data-toggle="tooltip" title="' + subdd[0]+'('+subdd[1] + '">'+subdd[0]+'<i class="fa fa-envelope" aria-hidden="true" style="color:white"></i>('+subdd[1]+'</span><br>';
            ...
            {data: 'company_email_add', name:'company_email_add',"render":function(data,type,full,meta){
                   ...
                   // var obj=data.replace(/,/g,"</br>");   
                        ...
                        for (i = 0; i < obj.length; i++) {
                          ...
                          var dd="'"+obj[i]+"'";
                            // var tt=dd.replace("@", "bb");
                          text1 += '<span name=emailtext[] value="'+cemail+'"><i class="fa fa-envelope" aria-hidden="true" style="font-size:25px;color:white"></i>&nbsp;&nbsp;&nbsp;('+subdd[1]+'</span> <br>';
          ...
          @permission('view.company.assign.user')
            ...
            {data:'user_assign_name',name:'user_assign_name',class:'assignuserclass dt-center',"render":function(data,type,full,meta){
              return '<span onclick="assignSingleUser('+full.id+')">'+data+'</span>';
            }},
          @endpermission
          @permission('view.company.assign.by')
            {data:'assign_by',name:'company_masters.assign_by',class:' dt-center'},
        
            // {data: 'industry', name: 'company_masters.industry'}, 
            // {data:'type_business',name:'company_masters.type_business'},
          @endpermission
...
$('.filternewdialaccont').on('click', function(){
  ...
  var new_url = "{{ route('company.asssignanydata',':id') }}";
