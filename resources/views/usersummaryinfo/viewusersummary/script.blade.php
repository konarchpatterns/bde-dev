<script type="text/javascript">
	$(function () {
     var table = $('#usersummaryrecord_a').DataTable({
        processing: true,
        serverSide: true,
        async: true,
        responsive: true,
       ...
       //  scrollX: true,//scroll horizontally
        ...
        ajax: "{{ route('usersummary.anydata') }}",
        columnDefs: [
          ...
          columns: [ 
               ...
               {data: 'todaycountdisposition', name: 'todaycountdisposition'},
              //  {data: 'flag', name:'flag',    "render": function (data, type, full, meta) {
              //               if ( data =='1')
              //               {
              //                     return '<span class="testact" data-toggle="tooltip">' + data + '</span>';
              //               }
              //               else{
              //                       return '<span class="testactin" data-toggle="tooltip">' + data + '</span>';
              //               }
              //             }
                         
              // },
               {data:'currnetlyallocated',name:'assignusers.id',class:"assigncompanyname dt-center"}, 
               {data:'unallocated',name:'unallocated',class:"unallocated dt-center","render":function(data,type,full,meta){
                   return full.totalallocatedd - full.currnetlyallocated;
               }}, 
               {data:'totalallocatedd',name:'assignusers.id',class:"totalallocated dt-center"},     
           ], 
           order: [[2, 'asc']]

      });
       table.on( 'draw.dt', function () {
       var PageInfo = $('#usersummaryrecord_a').DataTable().page.info();
  ...
  //  inactive user
  $(function () {
     var table = $('#usersummaryrecord_i').DataTable({
        processing: true,
        serverSide: true,
        async: true,
        responsive: true,
       ...
       //  scrollX: true,//scroll horizontally
        ...
        "language": {
          ...
          //Import data in datatable
        ajax: "{{ route('usersummary.anydatai') }}",
        columnDefs: [
               {className: "dt-center", targets: "_all"},
               {searchable: false,
               orderable: false,
       ...
       table.on( 'draw.dt', function () {
       var PageInfo = $('#usersummaryrecord_i').DataTable().page.info();
         table.column(0, { page: 'current' }).nodes().each( function (cell, i) {
            cell.innerHTML = i + 1 + PageInfo.start;
         });
       });
  ...
  //bde user 
  $(function () {
     var table = $('#bdeusersummaryrecord').DataTable({
        processing: true,
        serverSide: true,
        async: true,
        responsive: true,
       ...
       //  scrollX: true,//scroll horizontally
        ...
        "language": {
          ...
          //Import data in datatable
        ajax: "{{ route('usersummary.bdeanydata') }}",
        columnDefs: [
               {className: "dt-center", targets: "_all"},
               {searchable: false,
               orderable: false,
